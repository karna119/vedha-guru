...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart, language, setLanguage }) => {
    ...
    return (
        <div className="min-h-screen relative overflow-x-hidden bg-gradient-to-br from-orange-50 to-amber-50" onClick={() => {
            ...
            {/* Audio Element - Direct Wikimedia URL */}
            <audio ref={audioRef} src="https://upload.wikimedia.org/wikipedia/commons/7/77/Om.ogg" preload="auto" />
            ...
            <button
                ...
                className="fixed bottom-4 right-4 z-50 bg-white/80 backdrop-blur p-3 rounded-full shadow-lg border border-orange-200 hover:scale-110 transition-transform flex items-center gap-2 group"
                title="Play/Pause Chant"
            >
                <span className="text-xl">🕉️</span>
                <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 text-sm font-medium whitespace-nowrap">Play Omkara</span>
            </button>
            ...
            <header className="relative min-h-[90vh] md:min-h-screen flex flex-col justify-center items-center text-center px-4 overflow-hidden pt-16 md:pt-0">
                ...
                <div className="absolute inset-0 z-0">
                    <img
                        src="https://images.unsplash.com/photo-1476304884326-cd2c88572c5f?q=80&w=1920&auto=format&fit=crop"
                ...
                <div className="relative z-10 max-w-5xl mx-auto">
                    <h2 className="text-4xl sm:text-5xl md:text-7xl font-bold text-white mb-6 leading-tight drop-shadow-lg px-2">
                        {t.heroHeadline}
                    </h2>
                    <p className="text-lg sm:text-xl md:text-2xl text-orange-50 font-medium max-w-3xl mx-auto mb-10 leading-relaxed drop-shadow-md px-4">
                        {t.heroSubheading}
                    </p>
                    ...
                    <div className="mt-4 md:mt-8">
                        <button
                            onClick={() => onStart()}
                            className="w-full sm:w-auto px-8 py-4 md:px-10 md:py-5 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-full font-bold text-lg md:text-xl shadow-2xl hover:scale-105 active:scale-95 transition-transform border-2 border-orange-300/30 ring-4 ring-orange-500/20"
                        >
                            {t.beginJourney}
                        </button>
                    </div>
            ...
            {/* Vision Section */}
            <section className="py-16 md:py-24 bg-gradient-to-b from-orange-50 to-white relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
                    <h3 className="text-3xl md:text-4xl font-bold mb-8 text-orange-900">{t.visionTitle}</h3>
                    <div className="max-w-4xl mx-auto">
                        <blockquote className="text-2xl md:text-3xl font-serif italic text-orange-700/80 mb-8 md:mb-10 leading-relaxed px-2">
                            "{t.visionStatement1}"
                        </blockquote>
                        <p className="text-lg md:text-xl text-gray-600 mb-12 px-2">{t.visionStatement2}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mt-12 md:mt-16">
                        {(t.whyDifferentPoints || []).map((point, idx) => (
                            <div key={idx} className="bg-white p-6 rounded-2xl shadow-lg border border-orange-100 hover:shadow-xl transition-shadow flex flex-col items-center">
                                <span className="text-3xl md:text-4xl block mb-4">🕉️</span>
                                <p className="text-base md:text-lg text-gray-700 font-medium">{point}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* Footer */}
            <footer className="bg-gray-50 py-8 md:py-12 text-center border-t border-gray-100">
                <p className="text-sm md:text-base text-gray-400">© 2025 DharmaSaathi. All rights reserved.</p>
            </footer>
        </div>
    );
};
